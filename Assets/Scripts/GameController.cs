...
public class GameController : MonoBehaviour {
    ...
    void Start () {
		...
		UpdateScore();
        record.text =  "" + PlayerPrefs.GetInt("record", 0);
    ...
    public void Update()
    {
        ...
        if (playerScore > PlayerPrefs.GetInt("record", 0))
        {
            PlayerPrefs.SetInt("record", playerScore);
            recordPuntos = playerScore;
            record.text = recordPuntos.ToString();
        }
    ...
    IEnumerator WaitForAnimationThenContinue(GameObject clicked, int maxDepth)
    {
        ...
        currentPlayerText.text = "Jugador " + (currPlayer + 1);
    ...
    void UpdateScore() {
		...
		if (GameModel.IS_HUMAN) {
            ...
            // La puntuación frente a la IA depende de la dificultad.
    ...
    void CheckVictory()
    {
        // Comprueba que todos los nodos sean neutrales o pertenezcan al jugador actual.
		...
		if (GameModel.IS_HUMAN) {
			endPopup.GetComponentsInChildren<Text> () [1].text = "Jugador " + (currPlayer + 1) + " gana!";
		} else {
			if (currPlayer == 0) {
				endPopup.GetComponentsInChildren<Text>()[1].text = "¡Tú ganas! Has eliminado la amenaza del virus Puntuación:" + playerScore;
               

            } else {
				endPopup.GetComponentsInChildren<Text>()[1].text = "La infección del virus se ha propagado. El centro de datos se ha perdido. Estás despedido :(";
			}
		}
